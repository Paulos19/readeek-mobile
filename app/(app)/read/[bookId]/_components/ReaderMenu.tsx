...
export const ReaderMenu = ({
    ...
    const renderSettings = () => (
        <View className="p-4 space-y-6">
            <View className="bg-zinc-800/80 p-4 rounded-xl flex-row items-center justify-between border border-white/10 mb-6">
                <TouchableOpacity onPress={() => onChangeFont(-10)} className="w-12 h-12 bg-zinc-700/80 rounded-lg items-center justify-center active:bg-zinc-600"><Minus color="#fff" size={24}/></TouchableOpacity>
                <Text className="text-white font-bold text-2xl tracking-widest">{fontSize}%</Text>
                <TouchableOpacity onPress={() => onChangeFont(10)} className="w-12 h-12 bg-zinc-700/80 rounded-lg items-center justify-center active:bg-zinc-600"><Plus color="#fff" size={24}/></TouchableOpacity>
            </View>
            <View className="flex-row gap-3">
                {['light', 'sepia', 'dark'].map((t) => {
                    ...
                    return (
                        <TouchableOpacity key={t} onPress={() => onChangeTheme(t)} className={`flex-1 py-4 rounded-xl border-2 items-center justify-center ${isSelected ? 'border-emerald-500 bg-zinc-800/90' : 'border-zinc-800 bg-zinc-900/50'}`}>
    ...
    const renderChapters = () => (
        <FlatList
            data={toc} key={2} numColumns={2}
            contentContainerStyle={{ padding: 16, gap: 12, paddingBottom: 100 }} columnWrapperStyle={{ gap: 12 }}
            keyExtractor={(i, index) => index.toString()}
            renderItem={({ item, index }) => (
                <TouchableOpacity onPress={() => onSelectChapter(item.href)} className="flex-1 aspect-[3/4] bg-zinc-800/50 rounded-lg border border-white/5 p-4 justify-between active:bg-zinc-700/50 overflow-hidden relative">
                    <View className="absolute top-2 right-2 opacity-10"><Text className="text-4xl font-serif font-bold text-white">{index + 1}</Text></View>
                    <View className="space-y-1 opacity-20"><View className="h-1 bg-white w-full rounded-full" /><View className="h-1 bg-white w-5/6 rounded-full" /><View className="h-1 bg-white w-4/6 rounded-full" /></View>
                    <View><Text className="text-zinc-500 text-[10px] uppercase font-bold tracking-widest mb-1">Capítulo {index + 1}</Text><Text className="text-zinc-200 font-serif text-sm font-medium leading-tight" numberOfLines={3}>{item.label ? item.label.trim() : `Parte ${index + 1}`}</Text></View>
                </TouchableOpacity>
            )}
    ...
    const renderHighlights = () => (
        <FlatList
            data={highlights} keyExtractor={(i) => i.id} contentContainerStyle={{ padding: 16 }}
            renderItem={({ item }) => {
                    ...
                    <View className="mb-3 p-3 rounded-lg bg-zinc-900/50 border border-white/5 flex-row gap-3 items-start">
                        ...
                        <View className="flex-1">
                            ...
                            <View className="flex-row justify-between items-center">
                                <View className="flex-row items-center gap-1"><Bookmark size={10} color={displayColor} /><Text className="text-zinc-600 text-[10px] uppercase font-bold">Marcador</Text></View>
                                <TouchableOpacity onPress={() => onDeleteHighlight(item.cfiRange, item.id)} className="p-1.5 bg-red-500/10 rounded border border-red-500/20"><Trash2 color="#ef4444" size={14} /></TouchableOpacity>
                            </View>
                        </View>
                    </View>
                );
        ...
        <>
            <View className="absolute top-0 left-0 right-0 h-28 bg-zinc-950/90 flex-col justify-end px-4 pb-4 z-50 border-b border-white/5 backdrop-blur-md">
                <View className="flex-row items-center justify-between mb-4">
                    ...
                    <Text className="text-zinc-200 text-sm font-semibold text-center flex-1 mx-2" numberOfLines={1}>{title}</Text>
                ...
                <View className="w-full bg-zinc-800 h-1.5 rounded-full overflow-hidden">
                    <View className="bg-emerald-500 h-full rounded-full" style={{ width: `${Math.round(progress * 100)}%` }} />
                </View>
            ...
            <Animated.View className={`absolute bottom-0 left-0 right-0 bg-zinc-900 rounded-t-3xl shadow-2xl z-50 border-t border-white/10 overflow-hidden`} style={animatedStyle}>
                ...
                {!expanded && (
                    <View className="p-6 flex-row justify-around items-center h-[140px]">
                        <TouchableOpacity onPress={() => handleTabPress('settings')} className="items-center gap-2 p-2">
                        ...
                        <TouchableOpacity onPress={() => handleTabPress('chapters')} className="items-center gap-2 p-2">
                            <View className="w-14 h-14 bg-zinc-800/80 rounded-2xl items-center justify-center border border-white/5 active:bg-zinc-700"><LayoutGrid color="#e4e4e7" size={24} /></View>
                            <Text className="text-zinc-500 text-xs font-bold">Capítulos</Text>
                        </TouchableOpacity>
                        <TouchableOpacity onPress={() => handleTabPress('highlights')} className="items-center gap-2 p-2">
                ...
                {expanded && (
                    <View className="flex-1">
                        <View className="flex-row border-b border-white/5">
                            {[{ id: 'settings', label: 'Ajustes', icon: Type }, { id: 'chapters', label: 'Capítulos', icon: LayoutGrid }, { id: 'highlights', label: 'Marcadores', icon: Highlighter }].map((tab) => (
                                <TouchableOpacity key={tab.id} onPress={() => onSetTab(tab.id as any)} className={`flex-1 flex-row items-center justify-center py-4 gap-2 ${activeTab === tab.id ? 'border-b-2 border-emerald-500 bg-white/5' : ''}`}>
